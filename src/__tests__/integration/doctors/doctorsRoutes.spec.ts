...
import { mockedDoctors, mockedMissingProperties, mockedInvalidAge, mockedInvalidCrm, mockedInvalidStates, mockedNulledProperties } from "../../mocks/doctors/index"
...
describe("Testing /DOCTORS routes", () => {
  ...
  test("POST /doctors - Should not be able to create a Doctor with invalid CRM. CRM must have 2 letters and 6 numbers", async () => {
    const response = await request(app).post("/doctors").send(mockedInvalidCrm[0])
    expect(response.status).toBe(400)
    expect(response.body).toHaveProperty("message")

    const response2 = await request(app).post("/doctors").send(mockedInvalidCrm[1])
    expect(response.status).toBe(400)
    expect(response.body).toHaveProperty("message")
  })

  test("POST /doctors - Should not be able to create a Doctor with invalid Age. Age cannot be 0 or greater than 99", async () => {
    const response = await request(app).post("/doctors").send(mockedInvalidAge[0])
    expect(response.status).toBe(400)
    expect(response.body).toHaveProperty("message")

    const response2 = await request(app).post("/doctors").send(mockedInvalidAge[1])
    expect(response.status).toBe(400)
    expect(response.body).toHaveProperty("message")
  })

  test("POST /doctors - Should not be able to create a Doctor with invalid States. State must be a string with length = 2", async () => {
    const response = await request(app).post("/doctors").send(mockedInvalidStates[0])
    expect(response.status).toBe(400)
    expect(response.body).toHaveProperty("message")

    const response2 = await request(app).post("/doctors").send(mockedInvalidStates[1])
    expect(response.status).toBe(400)
    expect(response.body).toHaveProperty("message")
  })
  ...
  test("GET /doctors/profile - Should be able to fetch information of the logged doctor", async () => {
    ...
    const response = await request(app).get(`/doctors/profile`).set("Authorization", `Bearer ${token}`)
    expect(response.status).toBe(200)
  ...
  test("DELETE /doctors/:id - Should be able to soft delete target doctor", async () => {
    ...
    const response = await request(app).delete(`/doctors/${loggedDoctor!.id}`).set("Authorization", `Bearer ${token}`)
  ...
  test("DELETE /doctors/:id - Should not be able to soft delete target doctor with invalid ID", async () => {
    ...
    const response = await request(app).delete(`/doctors/fwepfwpefokwpfokpq3okfpokf`).set("Authorization", `Bearer ${token}`)
  ...
  const proibido = ["id", "CRM", "specialties", "isActive", "createdAt", "updatedAt"]
