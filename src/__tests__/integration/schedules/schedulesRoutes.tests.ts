...
describe("/schedules", () => {
	...
	test("PATCH /schedules/edit/:id - DOCTORS -  Should not be able to edit schedule without authentication", async () => {			
		...
		const schedulesToBeEdit = await request(app).get("/schedules/doctors")
	...
	test("PATCH  /schedules/edit/:id -  DOCTORS - Should not be able to edit schedule with invalid id", async () => {
		...
		const response = await request(app).patch(`/schedules/edit/13970660-5dbe-423a-9a9d-5c23b37943cf`)
	...
	test("PATCH /schedules/:id -  USER - Should not be able to schedule without authentication", async () => {
		...
		const schedulesToBeScheduleByUser = await request(app).get(`/doctors/${doctor.body[0].id}/schedules`)
	...
	test("PATCH  /schedules/:id -  USER - Should not be able to schedule with invalid id", async () => {
		...
		const response = await request(app).patch(`/schedules/13970660-5dbe-423a-9a9d-5c23b37943cf`)
		.set("Authorization", `Bearer ${loginUserResponse.body.token}`)
	...
	test("PATCH  /schedules/:id - USER -  Must be able to schedule ", async () => {
		...
		const scheduleToBeEdit = await request(app).get(`/doctors/${doctor.body[0].id}/schedules`)
		.set("Authorization", `Bearer ${loginUserResponse.body.token}`);

		const response = await request(app).patch(`/schedules/${scheduleToBeEdit.body[0].id}`)
		.set("Authorization", `Bearer ${loginUserResponse.body.token}`)
	...
	test("DELETE /schedules/:id -  USER - Should not be able to cancel a schedule without authentication", async () => {
		...
		const schedulesToBeCancelByUser = await request(app).get(`/doctors/${doctor.body[0].id}/schedules`)
		.set("Authorization", `Bearer ${loginUserResponse.body.token}`);
	...
	test("DELETE /schedules/delete/:id -  USER ADMIN - Should not be able to delete a schedule without authentication", async () => {
		const loginUserResponse = await request(app).post("/login/users").send(mockedAdminLogin);

		const doctor = await request(app).get("/doctors")

		const scheduleToBeDeleted = await request(app).get(`/doctors/${doctor.body[0].id}/schedules`)
		.set("Authorization", `Bearer ${loginUserResponse.body.token}`)
	...
	test("DELETE /schedules/delete/:id-  USER ADMIN - Should not be able to delete schedule with invalid id", async () => {
		const loginResponse = await request(app).post("/login/users").send(mockedAdminLogin);

		const response = await request(app).delete(`/schedules/delete/13970660-5dbe-423a-9a9d-5c23b37943cf`)
		.set("Authorization", `Bearer ${loginResponse.body.token}`)
